...
export function StatusBar({ config, status, onSettingsClick }: StatusBarProps) {
  ...
  const networkAnchorRef = useRef<HTMLButtonElement>(null)
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative flex items-center gap-1">
          <button
            ...
            type="button"
            onClick={() => setNetworkInfoOpen((v) => !v)}
            className="rounded-lg border border-gray-700 bg-gray-800/80 p-2 text-gray-400 transition hover:border-gray-500 hover:text-white"
            title="Network (two networks doc)"
          >
          ...
          <NetworkInfo
            config={config}
            open={networkInfoOpen}
            onClose={() => setNetworkInfoOpen(false)}
            anchorRef={networkAnchorRef}
